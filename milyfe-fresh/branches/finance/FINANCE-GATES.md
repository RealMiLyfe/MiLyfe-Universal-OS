...
1. **value states explicit** — nine labels (projected/pending/verified/settled/allocated/rewarded/reinvested/reserved/disputed/reversed) labeled everywhere; projected never spendable/shown-as-balance (Money-State Contract).
...
8. **USD-facing rails remain locked until risk/disclosure/partner review + evidence + human approval** — locked-capability register (MiScale): USD rails, reverse automation, exchange listing, fiat POS, plastic, new assets. (Internal items — welcome budgets, trusted-node policy — need human + safety/treasury approval, never outside permission.)
