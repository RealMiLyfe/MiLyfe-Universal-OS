...
export function VotingLedger({ votes }: Props) {
 ...
 {vote.direction === 'for' ? (
 <ThumbsUp className="h-3 w-3 text-green-600 " aria-hidden="true" />
 ) : (
 <ThumbsDown className="h-3 w-3 text-red-500 " aria-hidden="true" />
 )}
 </div>
 ...
 vote.direction === 'for' ? 'text-green-600 ' : 'text-red-500 '
 }`}>
 ...
 className="w-full flex items-center justify-center gap-1 px-5 py-3 text-sm text-teal-600 hover:bg-gray-50 transition-colors border-t border-gray-100 "
 >
