...
export async function POST(request: Request) {
  ...
  const apiBase = process.env.GROQ_API_KEY
    ? 'https://api.groq.com/openai/v1'
    : (process.env.OPENAI_API_BASE_URL || 'http://localhost:11434/v1');
  const apiKey = process.env.GROQ_API_KEY || process.env.OPENAI_API_KEY || 'ollama';
  const model = process.env.MI_MODEL || (process.env.GROQ_API_KEY ? 'llama-3.1-8b-instant' : 'llama3.2:3b');
