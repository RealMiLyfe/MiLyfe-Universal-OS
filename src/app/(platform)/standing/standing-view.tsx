...
export function StandingView({ standing, attestations, profile }: Props) {
 ...
 <div className="inline-flex items-center justify-center h-20 w-20 rounded-full bg-teal-50 mb-3">
 <Star className="h-10 w-10 text-teal-500" aria-hidden="true" />
 </div>
 <p className="text-3xl font-bold text-harbor-800 ">
 ...
 <span className="text-sm font-medium text-harbor-800 ">{label}</span>
 </div>
 <span className="text-sm font-bold text-harbor-800 ">{value.toFixed(1)}</span>
 </div>
 ...
 <li key={att.id} className="flex items-start gap-3 py-2 border-b border-gray-50 last:border-0">
 <Avatar
 ...
 <p className="text-xs text-gray-600 mt-0.5">{att.reason}</p>
