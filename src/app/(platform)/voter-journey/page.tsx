  ...
  return (
    <div className="max-w-2xl mx-auto py-8 px-4">
      ...
      <div className="text-center mb-10">
        <div className="inline-flex p-3 rounded-full bg-teal-50 dark:bg-teal-900/20 mb-4">
          ...
          <div
            ...
            className={`rounded-xl border p-5 transition-colors ${
              completedSteps.includes(step.number)
                ...
                : 'border-gray-200 dark:border-harbor-700 bg-white dark:bg-harbor-900'
            }`}
          >
            <div className="flex items-start gap-4">
              <div className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center text-lg ${
                completedSteps.includes(step.number)
                  ...
                  : 'bg-gray-100 dark:bg-harbor-800'
              }`}>
                {completedSteps.includes(step.number) ? <CheckCircle className="h-5 w-5" /> : step.icon}
              </div>
              <div className="flex-1">
                <h3 className="font-bold text-harbor-800 dark:text-white mb-1">
                ...
                {step.dates && (
                  <div className="bg-gray-50 dark:bg-harbor-800 rounded-lg p-3 mb-3">
                    <div className="flex items-center gap-2 mb-2">
                      <Calendar className="h-4 w-4 text-mly-600" />
                      <span className="text-xs font-bold text-harbor-800 dark:text-white uppercase">Key Dates</span>
                    </div>
                    <div className="space-y-1">
                      {KEY_DATES.map((d) => (
                        <div key={d.date} className="flex justify-between text-xs">
                          <span className="text-gray-600 dark:text-gray-400">{d.event}</span>
                          <span className="font-medium text-harbor-800 dark:text-white">{d.date}</span>
                        </div>
                      ))}
                    </div>
                  </div>
      ...
      <div className="mt-8 text-center">
        <div className="bg-gray-50 dark:bg-harbor-800 rounded-xl p-5">
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">
            <strong className="text-harbor-800 dark:text-white">Why this matters:</strong> Jacksonville spends 52% of its budget reacting to problems. Your vote decides whether we keep doing that — or invest in prevention. MiLyfe built the proof. Your vote makes it real.
          </p>
          <Link href="/home">
