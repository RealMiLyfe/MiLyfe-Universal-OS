...
export function ChatThread({ currentUserId, otherUser, initialMessages }: Props) {
 ...
 <Link
 href="/connect"
 className="flex items-center justify-center h-9 w-9 rounded-lg hover:bg-gray-100 transition-colors shrink-0"
 ...
 <p className="text-sm font-semibold text-harbor-800 truncate leading-tight">
 ...
 <div>
 <p className="font-semibold text-harbor-800 ">{otherUser.display_name}</p>
 ...
 : cn('bg-gray-100 text-harbor-800 ',
 isFirstInRun && isLastInRun ? 'rounded-2xl' :
 ...
 rows={1}
 inputMode="text"
 className="flex-1 resize-none rounded-2xl border border-gray-200 bg-gray-50 px-4 py-2.5 text-sm text-harbor-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all min-h-[40px] max-h-[120px]"
 style={{ height: '40px' }}
 autoComplete="off"
 autoCorrect="on"
 spellCheck
 ...
 body.trim() ? 'bg-teal-500 hover:bg-teal-600 text-white shadow-md shadow-teal-500/25' : 'bg-gray-100 text-gray-400'
 )}
 >
