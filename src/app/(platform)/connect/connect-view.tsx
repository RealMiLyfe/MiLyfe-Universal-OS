...
export function ConnectView({ userId, connections, pendingRequests, conversations: initialConversations, unreadCount: initialUnread, allPeople }: Props) {
 ...
 className={cn(
 'flex items-center gap-3 px-4 py-3.5 transition-colors active:bg-gray-100 ',
 ...
 />
 {isUnread && (
 <span className="absolute -top-0.5 -right-0.5 h-3.5 w-3.5 rounded-full bg-teal-500 border-2 border-white " aria-hidden="true" />
 )}
 </div>
 ...
 <p className={cn('text-sm truncate', isUnread ? 'font-semibold text-harbor-800 ' : 'font-medium text-harbor-800 ')}>
 {name}
 </p>
 <span className="text-[11px] text-gray-400 shrink-0">{timeAgo}</span>
 </div>
 <p className={cn('text-xs truncate mt-0.5', isUnread ? 'text-harbor-700 ' : 'text-gray-500 ')}>
 {preview}
 </p>
 </div>
 ...
 <div className="rounded-xl border border-mly-200 bg-mly-50/50 p-4 mb-4">
 <p className="text-sm font-semibold text-harbor-800 mb-3 flex items-center gap-2">
 Pending Requests
 ...
 <p className="text-sm font-medium text-harbor-800 truncate">{req.requester?.display_name}</p>
 ...
 return (
 <div key={conn.id} className="flex items-center gap-3 rounded-xl border border-gray-100 bg-white px-4 py-3">
 <Avatar name={other?.display_name || 'U'} src={other?.avatar_url} />
 <div className="flex-1 min-w-0">
 <p className="text-sm font-medium text-harbor-800 truncate">{other?.display_name}</p>
 <p className="text-xs text-gray-500">@{other?.username}{other?.neighborhood ? ` · ${other.neighborhood}` : ''}</p>
 </div>
 <Link href={`/connect/${other?.id}`}>
 <Button size="sm" variant="ghost" className="text-teal-600 hover:bg-teal-50 " aria-label={`Message ${other?.display_name}`}>
 ...
 <div key={person.id} className="flex items-center gap-3 rounded-xl border border-gray-100 bg-white px-4 py-3">
 <Avatar name={person.display_name || 'U'} src={person.avatar_url} />
 <div className="flex-1 min-w-0">
 <p className="text-sm font-medium text-harbor-800 truncate">{person.display_name}</p>
