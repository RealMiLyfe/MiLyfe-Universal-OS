  ...
  if (mod) {
    ...
    if (isComplete) {
      ...
      if (path) {
        ...
        // Learning path completion milestone reward (50 $MLY)
        await supabase.from('rewards').insert({
          user_id: user.id,
          type: 'milestone',
          amount: 50,
          title: `Path Completed: ${path.title}`,
          description: `Earned for mastering all modules in the ${path.title} learning path!`,
          claimed: false,
        });
        ...
        await supabase.from('notifications').insert({
          user_id: user.id,
          type: 'reward',
          title: 'Path Reward: 50 $MLY Available',
          body: `You completed "${path.title}" and earned 50 $MLY. Claim your reward in Rewards!`,
          link: '/rewards',
        });
      }
    }
